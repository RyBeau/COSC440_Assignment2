...
def pre_process_data(inputs, labels, first_class, second_class):
    ...
    """
    indices_wanted = np.nonzero(np.logical_or(labels == first_class, labels == second_class))[0]
    filtered_labels = labels[indices_wanted]
    filtered_images = inputs[indices_wanted]
    converted_labels = np.where(filtered_labels == first_class, 0, 1)
    converted_labels = tf.one_hot(converted_labels, depth=2)
    reshaped_inputs = (np.reshape(filtered_images, (-1, 3, 32, 32)).transpose(0, 2, 3, 1) / 255).astype(np.float32)
    return reshaped_inputs, converted_labels
